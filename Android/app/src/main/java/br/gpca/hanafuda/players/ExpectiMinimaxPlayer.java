...
public class ExpectiMinimaxPlayer extends Player {
    ...
    public Action action(GameStates currentState, Card deckCard, Table table, Player opponent) {
        ...
        switch (currentState) {
            case MatchHandTable:
                return convertNodeInAction(getBestNode(NodeType.Max, table, this, opponent, maximumDepth));
                //game.matchCards(this, action.getCard1(), action.getCard2());
            case MatchDeckTable:
                Player p = new Player(game);
                this.earnedCards.cloneCards(p.earnedCards);
                p.hand.addCard(deckCard);
                return convertNodeInAction(getBestNode(NodeType.Max, table, p, opponent, maximumDepth));
                //game.matchCards(this, action.getCard1(), action.getCard2());
            case DiscardHandCard:
                return convertNodeInAction(getBestNode(NodeType.Max, table, this, opponent, (byte) 4));
                //game.matchCards(this, action.getCard1(), action.getCard2());
        }
